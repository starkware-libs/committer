...
impl FilledForestImpl {
    ...
    >(
        ...
        for (address, inner_updates) in storage_updates {
            ...
            let (new_contract_state, filled_storage_trie) = Self::new_contract_state::<T, H, TH>(
    ...
    >(
        ...
        old_contract_state: &ContractState,
    ) -> FilledTreeResult<(ContractState, FilledTreeImpl), LeafDataImpl> {
        let filled_storage_trie =
            FilledTreeImpl::create::<H, TH>(updated_storage_trie, inner_updates).await?;

        let new_nonce = new_nonce.unwrap_or(&old_contract_state.nonce);
        let new_class_hash = new_class_hash.unwrap_or(&old_contract_state.class_hash);
