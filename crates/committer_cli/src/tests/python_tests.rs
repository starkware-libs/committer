...
fn serialize_for_rust_committer_flow_test(input: HashMap<String, String>) -> String {
    let (leaf_modifications, storage, root_hash) = parse_input_single_tree_flow_test(input);
    // Serialize the leaf modifications to an object that can serialize.
    let leaf_modifications_to_print: HashMap<String, Vec<u8>> = leaf_modifications
        .into_iter()
        .map(|(k, v)| (k.0.to_string(), v.serialize().0))
        .collect();

    // crate a json to compare with the python expected.
