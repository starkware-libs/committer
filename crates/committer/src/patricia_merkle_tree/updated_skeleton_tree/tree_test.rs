...
async fn test_filled_tree_sanity() {
    ...
    let new_skeleton_leaf = SkeletonLeaf::CompiledClassHash(ClassHash(Felt::ONE));
    let new_filled_leaf = LeafDataImpl::CompiledClassHash(ClassHash(Felt::ONE));
    ...
    let modifications = new_leaves
        ...
        .map(|(index, value)| {
            (
                ...
                LeafDataImpl::CompiledClassHash(ClassHash(Felt::from_hex(value).unwrap())),
            )
        })
        .collect();
...
async fn test_small_tree_with_sibling_nodes() {
    ...
    let modifications = HashMap::from([(
        ...
        LeafDataImpl::CompiledClassHash(ClassHash(Felt::from_hex(new_leaf).unwrap())),
    )]);
...
fn create_leaf_updated_skeleton_node_for_testing(
    ...
    (
        NodeIndex::from(index),
        UpdatedSkeletonNode::Leaf(SkeletonLeaf::CompiledClassHash(ClassHash(
