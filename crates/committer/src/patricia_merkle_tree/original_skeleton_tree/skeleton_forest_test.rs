...
fn test_create_original_skeleton_forest(
    ...
    let (actual_forest, current_contracts_trie_leaves) = OriginalSkeletonForestImpl::<OriginalSkeletonTreeImpl>::create(
    ...
    .unwrap();
    let expected_current_contracts_trie_leaves = input.current_contracts_trie_leaves
        .into_iter()
        .map(|(address, state)| (NodeIndex::from_contract_address(&address), state))
        .collect();
    assert_eq!(
        current_contracts_trie_leaves,
        expected_current_contracts_trie_leaves
    );
