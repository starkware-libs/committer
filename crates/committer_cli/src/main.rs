...
fn main() {
    ...
    match args.command {
        Command::Commit {
            ...
            let node_prefixes_slice = [
                node_prefixes.edge,
                node_prefixes.sibling,
                node_prefixes.binary,
                node_prefixes.empty,
            ];
            if HashSet::from(node_prefixes_slice).len() != node_prefixes_slice.len() {
                panic!("Node prefixes must be unique, got {node_prefixes_slice:?}.");
            }
            let leaf_prefixes_slice = [
                leaf_prefixes.storage,
                leaf_prefixes.contract_state,
                leaf_prefixes.class,
            ];
            if HashSet::from(leaf_prefixes_slice).len() != leaf_prefixes_slice.len() {
                panic!("Leaf prefixes must be unique, got {leaf_prefixes_slice:?}.");
            }
