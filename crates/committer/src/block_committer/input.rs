...
impl StateDiff {
    ...
    pub(crate) fn actual_storage_updates(
        ...
        let mut actual_storage_updates = HashMap::new();
        for address in self.accessed_addresses() {
            let actual_inner_updates = match self.storage_updates.get(address) {
            ...
            actual_storage_updates.insert(*address, actual_inner_updates);
        }
