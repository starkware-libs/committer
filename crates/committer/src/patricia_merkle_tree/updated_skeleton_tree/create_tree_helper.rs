...
fn get_path_to_lca(
    ...
    let lca = if subtree_indices.len() == 1 {
        subtree_indices.get_indices()[0]
    } else {
        subtree_indices.get_indices()[0]
            .get_lca(subtree_indices.last().expect("Unexpected empty array"))
    };
...
impl UpdatedSkeletonTreeImpl {
    ...
    pub(crate) fn update_node_in_empty_tree(
        ...
        if root_index.is_leaf() {
            ...
            assert!(
                leaf_indices.len() == 1 && leaf_indices.get_indices()[0] == *root_index,
    ...
    pub(crate) fn update_node_in_nonempty_tree(
        ...
        if root_index.is_leaf() && leaf_indices.get_indices().contains(root_index) {
