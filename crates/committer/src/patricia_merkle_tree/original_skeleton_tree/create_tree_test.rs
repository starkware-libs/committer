...
fn test_create_tree(
    ...
    let leaf_modifications: HashMap<NodeIndex, StarknetStorageValue> = leaf_modifications
        .into_iter()
...
fn test_get_bottom_subtree(
    ...
    assert_eq!(
        abandoned_indices,
        create_abandoned_indices(&sorted_leaf_indices, &expected_sorted_leaf_indices)
    );
