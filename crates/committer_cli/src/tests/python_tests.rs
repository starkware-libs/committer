...
pub(crate) fn test_node_db_key() -> String {
    ...
    let binary_node_key = binary_node.get_db_key(&binary_node.suffix()).0;
    ...
    let edge_node_key = edge_node.get_db_key(&edge_node.suffix()).0;

    let storage_leaf = FilledNode {
        data: NodeData::Leaf(LeafDataImpl::StorageValue(zero)),
        hash,
    };
    let storage_leaf_key = storage_leaf.get_db_key(&storage_leaf.suffix()).0;
    ...
    let state_tree_leaf_key = state_tree_leaf.get_db_key(&state_tree_leaf.suffix()).0;

    let compiled_class_leaf = FilledNode {
        data: NodeData::Leaf(LeafDataImpl::CompiledClassHash(ClassHash(zero))),
        hash,
    };
    let compiled_class_leaf_key = compiled_class_leaf
        .get_db_key(&compiled_class_leaf.suffix())
        .0;
...
fn test_storage_node(data: HashMap<String, String>) -> Result<String, PythonTestError> {
    ...
    rust_fact_storage.set(
        binary_rust.get_db_key(&binary_rust.suffix()),
        binary_rust.serialize(),
    );
    ...
    rust_fact_storage.set(
        edge_rust.get_db_key(&edge_rust.suffix()),
        edge_rust.serialize(),
    );
    ...
    rust_fact_storage.set(
        storage_leaf_rust.get_db_key(&storage_leaf_rust.suffix()),
        storage_leaf_rust.serialize(),
    );
    ...
    rust_fact_storage.set(
        contract_state_leaf_rust.get_db_key(&contract_state_leaf_rust.suffix()),
    ...
    rust_fact_storage.set(
        compiled_class_leaf_rust.get_db_key(&compiled_class_leaf_rust.suffix()),
