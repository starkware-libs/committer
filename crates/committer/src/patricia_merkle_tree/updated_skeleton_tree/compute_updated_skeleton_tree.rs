        ...
        if *left != TempSkeletonNode::Empty && *right != TempSkeletonNode::Empty {
            ...
            let mut insert_updated = |index: NodeIndex, node: &TempSkeletonNode| {
                if let TempSkeletonNode::Original(original_node) = node {
                    let updated = match original_node {
                        OriginalSkeletonNode::EdgeSibling(edge_data) => {
                            self.skeleton_tree.insert(
                                edge_data.path_to_bottom.bottom_index(index),
                                UpdatedSkeletonNode::Sibling(edge_data.bottom_hash),
                            );
                        ...
                        OriginalSkeletonNode::Leaf(_) => {
                            let leaf = leaf_modifications
                                .get(&index)
                                .unwrap_or_else(|| panic!("Leaf modification {index:?} not found"));
                            if leaf.is_zero() {
                                panic!("Unexpected zero leaf.");
                            } else {
                                UpdatedSkeletonNode::Leaf(*leaf)
                            }
                        }
                    ...
                    panic!("Expected original node.")
                }
            };

            insert_updated(left_index, left);
            insert_updated(right_index, right);
