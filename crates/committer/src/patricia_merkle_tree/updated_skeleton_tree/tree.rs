...
use crate::patricia_merkle_tree::node_data::leaf::{LeafData, Modifications, SkeletonLeaf};
...
pub(crate) trait UpdatedSkeletonTree<L: LeafData>: Sized {
    ...
    fn create(
        ...
        index_to_updated_leaf: &Modifications<SkeletonLeaf>,
...
impl UpdatedSkeletonTreeImpl {
        ...
        TH: TreeHashFunction<L, H>,
    {
        ...
        match node {
            ...
            UpdatedSkeletonNode::Leaf(_) => {
                let data = NodeData::Leaf(
                    modifications
                        .get(&index)
                        .unwrap_or_else(|| {
                            panic!("Missing modification data for leaf with node index {index:?}.")
                        })
                        .clone(),
                );
                let hash_value = TH::compute_node_hash(&data);
...
impl<L: LeafData> UpdatedSkeletonTree<L> for UpdatedSkeletonTreeImpl {
    fn create(
        ...
        _index_to_updated_leaf: &Modifications<SkeletonLeaf>,
    ) -> Result<Self, UpdatedSkeletonTreeError<L>> {
        todo!()
    }
