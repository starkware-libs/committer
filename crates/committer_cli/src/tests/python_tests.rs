...
use starknet_api::core::{ClassHash as ApiClassHash, CompiledClassHash as ApiCompiledClassHash, ContractAddress as ApiContractAddress, EthAddress, Nonce as ApiNonce, PatriciaKey};
...
pub(crate) fn parse_state_diff_test(state_diff: ThinStateDiff) -> String {
    let expected_object = ThinStateDiff {
        ...
        storage_diffs: indexmap! {
            ...
            ApiContractAddress::from(10_u128) => indexmap! {
                ApiStorageKey::from(11_u128) => CoreFelt::from_bytes_be_slice(&[12_u8]),
            }
        },
        ...
        nonces: indexmap! {
            ...
            ApiContractAddress::from(5_u128) => ApiNonce(CoreFelt::from_bytes_be_slice(&[6_u8])),
        },
