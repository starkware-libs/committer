...
use crate::storage::storage_trait::{db_key, StorageKey, StorageValue};
...
impl FilledNode<LeafData> {
    ...
    pub(crate) fn db_key(&self) -> StorageKey {
        ...
        match &self.data {
            NodeData::Binary(_) | NodeData::Edge(_) => db_key(INNER_NODE_PREFIX, &suffix),
            NodeData::Leaf(LeafData::StorageValue(_)) => db_key(STORAGE_LEAF_PREFIX, &suffix),
            NodeData::Leaf(LeafData::CompiledClassHash(_)) => {
                db_key(COMPLIED_CLASS_PREFIX, &suffix)
            }
            NodeData::Leaf(LeafData::StateTreeTuple { .. }) => {
                db_key(STATE_TREE_LEAF_PREFIX, &suffix)
            }
        }
    }
}
