...
impl OriginalSkeletonTreeImpl {
    ...
    pub(crate) fn split_leaves<'a>(
        ...
        let first_leaf = leaf_indices[0];
        let root_height = self.get_node_height(root_index);
        if (first_leaf >> root_height.0) != *root_index {
            panic!("First leaf is not a descendant of the root.");
        }
        if leaf_indices.len() == 1 {
            return false;
        }
        ...
        let last_leaf = leaf_indices
            .last()
            .expect("leaf_indices unexpectedly empty.");
        if (*last_leaf >> root_height.0) != *root_index {
            panic!("Last leaf is not a descendant of the root.");
        }

        let right_child_index = (*root_index << 1) + 1.into();
        let leftmost_index_in_right_subtree = right_child_index << (root_height.0 - 1);
