...
/// consecutive numbers. If jump == 0, the maximal jump that won't cause an overflow is chosen.
fn create_increasing_random_array(size: usize, start: U256, mut jump: U256) -> Vec<U256> {
...
fn test_split_leaves_big_tree() {
    let left_leaf_indices =
        create_increasing_random_array(100, NodeIndex::_FIRST_LEAF.into(), U256::ONE << 200);
    let right_leaf_indices = create_increasing_random_array(
        100,
        (U256::from(NodeIndex::_FIRST_LEAF) + U256::from(NodeIndex::MAX_INDEX)) / 2 + 1,
        U256::ONE << 100,
    );
    test_split_leaves(
        TreeHeight::MAX.into(),
