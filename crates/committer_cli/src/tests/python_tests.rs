...
fn get_or_key_not_found(map: &HashMap<String, u128>, key: &str) -> Result<u128, PythonTestError> {
    Ok(*map.get(key).ok_or_else(|| {
        PythonTestError::KeyNotFound(
            "Failed to get value for key '{key}' from {map:?}.".to_string(),
        )
    })?)
}
...
fn test_storage_node(data: HashMap<String, String>) -> Result<String, PythonTestError> {
    ...
    let binary_json = data.get("binary").ok_or_else(|| {
    ...
    let edge_json = data.get("edge").ok_or_else(|| {
    ...
    let storage_leaf_json = data.get("storage").ok_or_else(|| {
    ...
    let contract_state_leaf = data.get("contract_state_leaf").ok_or_else(|| {
        PythonTestError::KeyNotFound(
            "Failed to get value for key 'contract state_leaf'".to_string(),
        )
    })?;
    ...
    let compiled_class_leaf = data.get("contract_class_leaf").ok_or_else(|| {
        PythonTestError::KeyNotFound(
