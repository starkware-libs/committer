...
impl PythonTest {
    ...
    pub(crate) fn run(&self, input: Option<&str>) -> Result<String, PythonTestError> {
        match self {
            ...
            Self::ParseBlockInfo => {
                let input: Value = serde_json::from_str(Self::non_optional_input(input)?)?;
...
pub(crate) fn parse_block_info_test(test_args: Value) -> String {
    let block_info: BlockInfo = serde_json::from_value(test_args).expect("error json");
    format!(
