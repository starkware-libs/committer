...
fn test_create_original_skeleton_forest(
    ...
    let mut classes_indices = create_expected_sorted_indices(&expected_classes_trie_sorted_indices);
    let sorted_classes_indices = SortedLeafIndices::new(&mut classes_indices);
    let copied_expected_classes_skeleton = create_original_skeleton_with_sorted_indices(
        sorted_classes_indices,
        &expected_forest.classes_trie,
    );
    assert_eq!(actual_forest.classes_trie, copied_expected_classes_skeleton);

    let mut contracts_indices =
        create_expected_sorted_indices(&expected_contracts_trie_sorted_indices);
    let sorted_contracts_indices = SortedLeafIndices::new(&mut contracts_indices);
    let copied_expected_contracts_skeleton = create_original_skeleton_with_sorted_indices(
        sorted_contracts_indices,
        &expected_forest.contracts_trie,
    );
    assert_eq!(
        actual_forest.contracts_trie,
        copied_expected_contracts_skeleton
    );

    for (contract, indices) in expected_storage_tries_sorted_indices {
        let mut leaf_indices = create_expected_sorted_indices(&indices);
        let sorted_indices = SortedLeafIndices::new(&mut leaf_indices);
        let contract_address = ContractAddress(Felt::from(contract));
        let copied_storage_trie = create_original_skeleton_with_sorted_indices(
            sorted_indices,
            expected_forest
                .storage_tries
