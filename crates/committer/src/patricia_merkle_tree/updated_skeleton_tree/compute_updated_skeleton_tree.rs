    ...
    fn update_edge_node(
            ...
            || (!left_indices.is_empty() && !was_left_nonempty)
        {
            ...
            if path_to_bottom.length.0 > 1 {
                // Bottom is not a child of the root, removing the first edge returns a valid new edge node.
            ...
            // else {
            //     // Bottom is a child of the root, removing the first edge returns the original node.
            //     let =
            //     assert!(self.skeleton_tree.get(&nonempty_subtree_child_index).is_some(), "Unexpectedly missing nonempty subtree child node.") &&
            //     let orig_nonempty_subtree_child =
            // }
