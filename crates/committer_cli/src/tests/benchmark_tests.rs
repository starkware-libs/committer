...
pub async fn test_benchmark_single_tree() {
    ...
    let output_map: TreeRegressionOutput = serde_json::from_str(&output).unwrap();
    let output_hash = output_map.root_hash;
    let expected_hash = regression_input.expected_hash;
    assert_eq!(output_hash, expected_hash);

    // Assert the storage changes.
    let Value::Object(storage_changes) = output_map.storage_changes else {
...
pub async fn test_benchmark_committer_flow() {
    ...
    // Assert the storage changes.
    // TODO(Aner, 8/7/2024): use structs for deserialization.
    let Value::Object(storage_changes) = committer_output.storage.get("storage").unwrap() else {
