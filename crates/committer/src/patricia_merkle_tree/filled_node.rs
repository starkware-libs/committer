    ...
    pub(crate) fn to_pre_image_js(&self) -> String {
        ...
        match &self {
            ...
            LeafData::CompiledClassHash(class_hash) => {
                ...
                serde_json::to_string(&temp_object_to_json)
                .expect("Failed to serialize class hash to JSON")
            }
    ...
    pub(crate) fn serialise(&self) -> SerialiseNode {
        match &self {
            LeafData::StorageValue(value) => {
                SerialiseNode::StorageLeaf(value.as_bytes())
            }
    ...
    pub(crate) fn serialise(&self,) -> SerialiseNode {
        ...
        match &self.data {
            NodeData::Binary(BinaryData {left_hash, right_hash}) => {
                ...
                let left: [u8; SERIALISE_HASH_SIZE] = left_hash.0.as_bytes();
                let right: [u8; SERIALISE_HASH_SIZE] = right_hash.0.as_bytes();
            ...
            NodeData::Edge(EdgeData {bottom_hash, path_to_bottom}) => {
                ...
                let bottom: [u8; SERIALISE_HASH_SIZE] = bottom_hash.0.as_bytes();
                let path: [u8; SERIALISE_HASH_SIZE] = path_to_bottom.path.0.as_bytes();
