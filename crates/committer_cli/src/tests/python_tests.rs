...
use ::committer::patricia_merkle_tree::utils::random_structs::DumpyRandomValue;
...
pub(crate) enum PythonTestError {
    ...
    #[error(transparent)]
    PrintOutputError(#[from] FilledForestError<LeafDataImpl>),
}
...
pub(crate) fn filled_forest_output_test() -> Result<String, PythonTestError> {
    let dumpy_forest = SerializedForest(FilledForestImpl::dumpy_random());
